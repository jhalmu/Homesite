defmodule HomesiteWeb.PortfolioLive.Index do
  ...
  import HomesiteWeb.MediaComponents, only: [template_icon: 1, template_name: 1]
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok,
     ...
     |> assign(:grouped_projects, group_projects_by_template(projects))
     |> assign(:page, 1)
  ...
  def handle_event("load_more", _params, socket) do
    ...
    {:noreply,
     ...
     |> assign(:grouped_projects, group_projects_by_template(all_projects))
            ...
            <div class="alert alert-info">
              ...
              <div>
                ...
                <div class="text-[var(--text-sm)]">
                  {gettext(
                    "No projects have been published yet. Add content or images to your projects to display them."
                  )}
                </div>
              </div>
            </div>
          <% else %>
            <%!-- Grouped gallery by template type --%>
            <div
              :for={{template_type, group_projects} <- @grouped_projects}
              class="mb-[var(--spacing-lg)]"
            >
              <h2 class="mb-[var(--spacing-sm)] gap-[var(--spacing-xs)] text-[var(--text-xl)] flex items-center font-semibold">
